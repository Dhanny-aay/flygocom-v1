    ...
    <>
      ...
      >
        <div className=" mt-8 w-full rounded-xl max-w-[640px] py-5 bg-[#0000004D] flex-col items-center justify-center flex text-center">
          ...
          <p className=" mt-3 text-xl font-Roboto font-normal text-white">
            Get everything you need for your trip on Flygocom.
          </p>
          <button className=" mt-3 block py-3 px-6 bg-[#1A8AF3] rounded-[5px] font-Roboto font-bold text-base text-white">
            See more
          </button>
        </div>
        ...
        >
          ...
          <div className=" w-full bg-white min-h-[257px] rounded-xl bg border border-[#EAEBF0] relative p-4">
            <img src={comvex} className=" absolute right-0 z-10 top-0" alt="" />
            <div className="w-full grid grid-cols-4 gap-6 z-20">
              {buttons.map((button, index) => (
                <button
                  key={index}
                  className={`py-3 px-12 rounded-[5px] flex items-center justify-center z-20 space-x-[10px] ${
