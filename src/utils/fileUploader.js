...
const FileUploader = ({ label, accept, maxSize, isImage, onFileSelect }) => {
  ...
  return (
    <label className="w-full flex flex-col mt-6 text-[#272D37] font-Outfit font-medium text-sm">
      {label}
      <div className="mt-2 w-full border border-[#EAECF0] rounded-[5px] flex items-center justify-center flex-col p-4">
        {isImage && imagePreview ? (
          ...
          <div className=" w-8 h-8 bg-[#F2F4F7] rounded-full flex items-center justify-center">
            <img src={pload} alt="Upload" />
          </div>
        )}

        <p className="mt-3 text-sm font-normal font-Roboto text-[#667085]">
          <span className="font-semibold text-[#0081FE] mr-1">
            Click to upload
          </span>{" "}
          or drag and drop
        </p>

        <p className="mt-1 text-xs font-normal font-Roboto text-[#667085]">
